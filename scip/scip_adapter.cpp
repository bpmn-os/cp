...
void SCIPSolver::addVariables(const Model& model) {
    for (const auto& var : model.getVariables()) {
        ...
        SCIP_VAR* scipVar;
        SCIPcreateVarBasic(scip_, &scipVar, var.name.c_str(),
                          var.lowerBound, var.upperBound, 0.0, vartype);
...
void SCIPSolver::addSequences(const Model& model) {
    for (const auto& seq : model.getSequences()) {
        // Get bounds from first variable (all sequence variables have same bounds)
        const Variable& firstVar = seq.variables[0];
        int minVal = static_cast<int>(firstVar.lowerBound);
        int maxVal = static_cast<int>(firstVar.upperBound);
        ...
        for (const auto& varRef : seq.variables) {
            ...
            SCIP_VAR* scipVar;
            SCIPcreateVarBasic(scip_, &scipVar, var.name.c_str(),
                              var.lowerBound, var.upperBound, 0.0,
        ...
        // Add sequence constraint (alldifferent permutation)
        addSequenceConstraints(scip_, seq.name, seqVars, minVal, maxVal);
    }
}
...
std::expected<SCIP_EXPR*, std::string> SCIPSolver::buildExpr(const Operand& operand) {
    ...
    if (std::holds_alternative<IndexedVariable>(operand)) {
        ...
        SCIPcreateVarBasic(scip_, &resultVar, resultName.c_str(),
                          -SCIPinfinity(scip_), SCIPinfinity(scip_),
                          0.0, SCIP_VARTYPE_CONTINUOUS);
        SCIPaddVar(scip_, resultVar);

        // Add element constraint (index offset is 0 since IndexedVariables uses 0-based indexing)
        std::string elemName = container.name + "[" + indexVar.name + "]";
        SCIP_EXPR* resultExpr = addElementConstraint(scip_, elemName, arrayVars,
                                                     scipIndexVar, resultVar, 0);
    ...
    if (std::holds_alternative<Expression>(operand)) {
        ...
        // Logical AND: a && b  (use min for booleans)
        ...
        if (cpExpr._operator == logical_or && cpExpr.operands.size() == 2) {
            ...
            SCIP_EXPR* prodChildren[] = { left.value(), right.value() };
            SCIPcreateExprProduct(scip_, &productExpr, 2, prodChildren, 1.0, nullptr, nullptr);

            SCIP_EXPR* expr;
            SCIP_EXPR* sumChildren[] = { left.value(), right.value(), productExpr };
            double coeffs[] = { 1.0, 1.0, -1.0 };
            SCIPcreateExprSum(scip_, &expr, 3, sumChildren, coeffs, 0.0, nullptr, nullptr);
        ...
        if (cpExpr._operator == custom && cpExpr.operands.size() >= 2) {
            ...
            if (opName == "min" || opName == "max") {
                ...
                SCIP_VAR* auxVar;
                SCIPcreateVarBasic(scip_, &auxVar, (opName + "_aux").c_str(),
                                  -SCIPinfinity(scip_), SCIPinfinity(scip_),
                                  0.0, SCIP_VARTYPE_CONTINUOUS);
                ...
                for (size_t i = 0; i < arrayExprs.size(); i++) {
                    ...
                    SCIPcreateVarBasic(scip_, &arrayVar, arrayVarName.c_str(),
                ...
                SCIP_EXPR* resultExpr = addElementConstraint(scip_, elemName, arrayVars, indexVar, resultVar, 0);
